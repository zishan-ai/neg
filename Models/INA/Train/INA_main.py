...
from get_intent import bert_model_buyer,bert_model_seller
import random
import math
import pdb
...
class Trainer():
    ...
    def random_split_data(self, data, next_intent, next_utter):
        ...
        train_data = [data[idx] for idx in indices[:120]] # XXXX: nuber of dialogues in train dataset
        val_data = [data[idx] for idx in indices[120:]]
        train_seller_intent = [next_intent[idx] for idx in indices[:120]]
        val_seller_intent = [next_intent[idx] for idx in indices[120:]]
        train_seller_utter = [next_utter[idx] for idx in indices[:120]]
        val_seller_utter = [next_utter[idx] for idx in indices[120:]]
    ...
    def intent_pred(self,current_sentence,pred_intent,role_id):
        ...
        for i in range(len(current_sentence)):
            if pred_intent[i]!='NA':
                idx = self.label2idx(pred_intent[i],role_id)

            if role_id ==0:
                intent_re = 0.54
                #prediction = bert_model_buyer()
                #intent_re = prediction.get_prediction(current_sentence[i],idx)
                intent_reward.append(intent_re)
                #return intent_reward
            if role_id == 1:
                intent_re=0.6
                #prediction = bert_model_seller()
                #intent_reward = prediction.get_prediction(current_sentence[i],idx)
                intent_reward.append(intent_re)
        return intent_reward
...
class NegotiationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.data[index])):
            ...
            dial_tokens.append(self.tokenizer.encode(self.data[index][i]) + self.tokenizer.encode(sep1) + self.tokenizer.encode(self.next_utter[index][i]) + self.tokenizer.encode(sep2) + self.tokenizer.encode(self.next_intent[index][i]) + self.turn_ending)
