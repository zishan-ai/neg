...
import math
import pdb
from rlutils_ninf import collect_samples, ppo_step, generate_n_candidates, convert_sentences_to_strings, expand_inputs_for_N_candidates
...
class Trainer():
    ...
    def extract_data(self, csvfile):
        ...
        for i in tqdm.trange(len(df_dialogs)):
            ...
            if line["speaker"] == 0:
                ...
                #print("Buyer")
                text = 'A:'+"buyer to seller: '" +line["utterance"].strip()
                ...
                text = "B:Seller to buyer: '" +line["utterance"].strip() 
    ...
    def random_split_data(self, data, next_intent, next_utter):
        ...
        train_data = [data[idx] for idx in indices[:2000]] # XXXX: nuber of dialogues in train dataset
        val_data = [data[idx] for idx in indices[2000:2500]]
        train_seller_intent = [next_intent[idx] for idx in indices[:2000]]
        val_seller_intent = [next_intent[idx] for idx in indices[2000:2500]]
        train_seller_utter = [next_utter[idx] for idx in indices[:2000]]
        val_seller_utter = [next_utter[idx] for idx in indices[2000:2500]]
...
if __name__ == '__main__':
    ...
    trainer = Trainer(modelname='model_v1_niinf',
                      ...
                      epsilon=0.2,
                      loadModel=False,
                      loadFilenameA="/model_v1_full/model_v1_full_A5700.pth",
                      loadFilenameB="/model_v1_full/model_v1_full_B5700.pth",
                      pad_token_id=2,
                      seedvalue=10, # 10 should be the seed value since pre trained on the same seed. 
                      use_intent=False,
                      use_prize_gap=False,
                      use_nego_strategy=False,
                      use_surface_similarity=False,
                      gamma1=0.2,
                      gamma2=0.3,
                      gamma3=0.3,
                      gamma4=0.2)
